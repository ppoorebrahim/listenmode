...
export default function EpisodePage() {
  const { playNewAudio, togglePlay, playing, audio } = useAudioPlayer();
  ...
  const handlePlay = () => {
    if (audio?.audioUrl === episodeData.audioUrl) {
      togglePlay();
    } else {
  ...
  const mobileCoverButtonSize = usePlayButtonSize("cover-mobile");
  const desktopCoverButtonSize = usePlayButtonSize("cover-desktop");
  ...
  return (
    <div className="flex flex-col md:flex-row gap-4 md:gap-4 px-4 md:px-8 pt-4 pb-24 bg-black text-white relative">
      ...
      <div className="md:hidden absolute top-0 left-0 right-0 flex items-center justify-between h-14 px-3 z-50 bg-transparent">
        ...
        <button className="p-2 bg-black/50 hover:bg-black/70 rounded-full backdrop-blur">
      ...
      <div className="flex-1 flex flex-col gap-4 md:gap-4 max-w-4xl pt-9 xs:pt-16">
        <div className="flex flex-col sm:flex-row gap-4 md:gap-3 items-start p-0 md:p-4 bg-[#1A1A1A] rounded-lg">
          ...
          <div className="relative w-full aspect-square sm:w-[250px] sm:h-[250px] bg-[#121212] overflow-hidden flex items-center justify-center rounded-lg">
            ...
            {/* Play button on Cover - Mobile */}
            <div className="absolute inset-0 flex items-center justify-center md:hidden">
              <PlayPauseButton
                isPlaying={audio?.audioUrl === episodeData.audioUrl && playing}
                onToggle={handlePlay}
                size={mobileCoverButtonSize}
              />
            </div>
        ...
        {/* Action Buttons */}
        <div className="flex flex-wrap justify-center md:justify-between items-center gap-2 mt-0">
